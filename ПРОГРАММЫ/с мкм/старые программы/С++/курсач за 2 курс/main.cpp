...
void Ris(Point *z, int ii, int jj)//изображение отрезка между точками zi и zj
{
	...
	SetPoint(z[ii - 1].x, z[ii - 1].y); // устанавливаем курсор на точку ii
	Line2(z[jj - 1].x, z[jj - 1].y);// строим отрезок,соединяющий курсор с  точкой jj, и перемещаем курсор в эту точку	
}
