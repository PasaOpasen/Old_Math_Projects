...
double m[o],n[y],z[o+y];
double a0,a1,b0,b1;
float scan;char pust[100];
int i,k=0,l=0,c;
clock_t BEGIN,END;
...
fscanf(fp,"%s",pust);fscanf(fp,"%s",pust);
	fscanf(fp,"%f",&scan);a0=scan;fscanf(fp,"%s",pust);
	fscanf(fp,"%f",&scan);a1=scan;fscanf(fp,"%s",pust);fscanf(fp,"%s",pust);
	fscanf(fp,"%f",&scan);b0=scan;fscanf(fp,"%s",pust);
	fscanf(fp,"%f",&scan);b1=scan;
...
//for(i=0;i<20;i++)fprintf(fpp," %2d",n[i]);
	for(i=0;i<20;i++)fprintf(fpp," %2f",z[i]);
}
