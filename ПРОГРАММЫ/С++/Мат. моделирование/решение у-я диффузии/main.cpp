...
void read()
{
	...
	for (int i = 1; i <= n; i++) f[i] = D2u(0 + i*hh);//заполнение свободного вектора значениями производной
...
{
	double m = abs(y[1] - u(1 * hh));
	cout<<m<<endl;

	for (int i = 2; i <= n; i++){ 
	cout<<abs(y[i] - u(i*hh))<<endl;
	if (abs(y[i] - u(i*hh)) > m) 
	m = abs(y[i] - u(i*hh));
		}
...
void RisL()
{
	...
	SetPoint(0,y[1]); // устанавливаем курсор на точку
	for(int i=2;i<=n+1;i++) Line2(i*hh,y[i]); 
