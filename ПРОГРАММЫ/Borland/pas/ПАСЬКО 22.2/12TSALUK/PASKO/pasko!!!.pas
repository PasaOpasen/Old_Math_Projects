...
BEGIN
  ...
  IF abs(s/q) < eps THEN
    IF abs(a.C - b.C) < eps THEN
      {Writeln('точки лежат на данной прямой')} PosPoints := FALSE
    ELSE
      {WriteLn('точки лежат по одну сторону от прямой')} PosPoints := FALSE
  ELSE
  BEGIN
    s1 := (-a.C*b.B + a.B*b.C)/s;
    s2 := (-a.A*b.C + b.A*a.C)/s;
    ...
    IF  s < eps THEN
      {WriteLn('одна из точек лежит на прямой')} PosPoints := FALSE
    ELSE IF ab < eps THEN
      {WriteLn('одна из точек лежит на прямой')} PosPoints := FALSE
    ELSE
      BEGIN
        ...
        ab :=  sqrt(sqr(P[1]-F[1])+sqr(P[2]-F[2]));
        IF abs(s-ab) < eps THEN
          {WriteLn('точки лежат по разные стороны от прямой')} PosPoints := TRUE
        ELSE
          {WriteLn('точки лежат по одну сторону от прямой')} PosPoints := FALSE
      END;
  END;
