...
Откорректируем классификацию, чтобы добиться стопроцентной точности:
```{r,cache=T}
acc=0#точность
repeat{ 
  showimage()
...
```
Самое важное здесь --- модель описывает более 80% дисперсий (**Adjusted R-squared**), каждый её коэффициент (**Pr(>|t|)**) и она сама (**p-value** в самом низу) статистически значимы. **Estimate** --- это коэффициенты модели, значит сама регресионная прямая имеет вид $$y=1098.57 + 61.93 t$$
