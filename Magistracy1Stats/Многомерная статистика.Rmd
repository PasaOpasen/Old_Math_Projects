...
Теперь попробуем визуально оценить качество кластеризации через главные компоненты:
```{r}
...
два лица уже похожи. Для четырёх кластеров имеется две пары очень похожих лиц (то есть данные разбиваются на два кластера, а дальнейшее разбиение уже излишнее):
```{r}
getfaces(4)
```
...
Тест Стьюдента для среднего значения показывает значимое отличие среднего ряда от нуля:
```{r}
...
ggplotly( ggplot(data,aes(x=time,y=values))+
  geom_point()+
...
приходим к выводу, что в целом поведение цен можно описать линейной моделью, однако для обычной линейной модели здесь явно не будет выполняться требование гомоскедастичности. Попробуем разные преобразования данных:
```{r}
summary(lm(y~x,df))
