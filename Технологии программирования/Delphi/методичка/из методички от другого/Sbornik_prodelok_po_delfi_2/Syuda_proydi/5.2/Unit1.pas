...
type
  TForm1 = class(TForm)
    ...
    procedure ButtonFloat2Click(Sender: TObject);
...
procedure TForm1.ButtonFloat2Click(Sender: TObject);
const symb: Set of Char = ['e','E','.','+','-','0'..'9'];
var str,st: String;
    i: Integer;
    flag,flagE: Boolean;
begin
  ...
  st:='';
  flag:=false;
  flagE:=false; //Если flagE равен true, то строка st содержит букву E
  LabelFloat2.Caption:='';
  for i:=1 to length(str) do
    if str[i] in symb then
    begin
      ...
      begin
        if flagE then
          try
            StrToFloat(st);
            LabelFloat2.Caption:=LabelFloat2.Caption+' '+st;
          except
          end;
        flag:=false;
        flagE:=false;
  ...
  if LabelFloat2.Caption = '' then
    LabelFloat2.Caption:='Строка не содержит веществ. чисел с плавающей точкой';
end;
